...
=\int_0^{z'}E[Q|X_{m_0}=p,Z_1=z_1,\cdots,Z_{m_1}=z_{m_1}]P[X_{m_0}]dp\;+\\\int_{z'}^1E[Q|X_{m_0}=p,Z_1=z_1,\cdots,Z_{m_1}=z_{m_1}]P[X_{m_0}]dp
\end{gather}
...
\newpage
\subsection{Application}
In section 4 we performed the counting of number of rejections after producing vectors of $p$-values. Now what we really need is those vectors. For each music feature, it has 1 corresponding vectors of $p$-values representing 997 times of hypothesis test. In the following, I will perform the Bonferroni correction and the Benjamini-Hochberg procedure on each of the $p$-vectors, and then compare the rejection counts to the original case.
